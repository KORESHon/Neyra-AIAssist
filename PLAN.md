...
| **Дашборд (frontend)** | Готово (мониторинг) | Vite/React: здоровье, память, список плагинов (read-only), баланс OpenRouter; **встроенного чата с Нейрой в UI пока нет** |
...
| **Не сделано (см. ниже)** | План | Управление плагинами из UI/API, вебхуки, чат в браузере, GitHub Pages, hardening безопасности инструментов, Voice/Screen/Music, десктоп/мобайл-клиенты |
...
- **Не сделано:** **включение/выключение и правка плагинов** из UI/API (нужен контракт: запись `plugin.yaml` и опционально per-plugin `config.yaml`; reload — отдельное решение).
- **Не сделано:** **вебхуки и микро-оповещения** (бывш. 2.3): настройка через UI; в ядре retry/backoff, throttling, dead-letter.
